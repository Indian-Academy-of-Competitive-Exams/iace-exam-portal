...
- **Anything that destroys, revokes, grants or changes what somebody can do asks first — in a `ConfirmDialog`, never a chip in the row.** Deleting a record, deactivating or reactivating a person, retiring a branch, creating an account that can sign in: one deliberate step, and the dialog **names what happens** rather than repeating the button. "Delete?" beside a row is a speed bump; it has no room to say that the group is how 240 students reach their tests, or that a super admin can create more super admins — and the whole reason to interrupt someone is the times that would change their answer. Include the number where there is one (`studentCount`, `groupCount`): a count is what turns a routine click into a decision. **Reversibility is not the test — visibility is.** Retiring a branch is undone by the button beside it, and it still asks, because nothing on that row changes except a badge while the consequence lands weeks later on somebody else trying to create a group. For the same reason **a toggle asks in BOTH directions**: a control that sometimes asks and sometimes does not is one people stop reading, and the reverse direction usually carries the fact nothing else says (reactivating an admin restores their sign-in but _not_ the grants that were dropped). `ConfirmDialog` already fixes the two things a call site gets wrong — Cancel takes the focus, and only the action is crimson — and it **stays open while the request is in flight** so a refusal lands on the dialog that caused it. The one standing exemption is an action whose screen **previews exactly what it would do** (committing an import): asking after showing the answer adds a step and no information. **Where confirming every click would be absurd, batch the clicks rather than drop the question** — the permission grid ticks a draft and saves the lot behind one dialog listing every change, because a dozen dialogs is how someone learns to click through dialogs without reading them. A batched editor holds its draft as a **diff against the server**, not a copy of it, so a background refetch cannot eat unsaved work; it shows the pending count somewhere still visible when the section is folded shut; and it drops the draft after a save **whether that saved or failed**, because a run that stopped halfway leaves the server as the only honest thing to render.
