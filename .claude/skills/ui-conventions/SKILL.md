...
- **A region with nothing to show is an `EmptyState`; an `Alert` is for data that IS there and needs attention.** A blue box saying "no events yet" is the wrong instrument — it reads as something to act on, and the reader hunts for the action. **The test is whether anything else renders**: an alert beside a figure that loaded, saying why one column is a dash, is right; an alert that IS the region is not. There are exactly **four** absences and the glyph names which one before a word is read: `EMPTY` (Inbox) nothing yet, `FILTERED` (SearchX) nothing matches, `FAILURE` (CircleX, crimson) it did not load, `REFUSED` (Lock, amber) you may not see it. `kind` picks the icon — **a per-screen glyph is decoration**, restating a heading that already said it, and `icon` is overridden only where the absence means something else ("No mistakes recorded" is a good outcome and takes a check).
  - **A failure carries its retry**, and it is the one absence that must: `<Alert variant="danger">did not load</Alert>` is a dead end whose only move left is reloading the page. `EmptyState` draws it from `onRetry`; a list gets it from `list.retry` with no call site involved.
  - **A refusal is not a failure.** Nothing broke and retrying changes nothing, so it is amber with no Retry — a permission boundary must not read as an outage. What it carries instead is how to be let in.
  - **A failure and a refusal name an EVENT, so their headings are sentences** — "Could not load this test", "This test is not open to you". That is the one exemption in `no-narration`; `EMPTY` and `FILTERED` name a region and stay plain nouns.
  - **A cell is not a region.** A dash in a table cell, a `BadgeList` with nothing in it, a `—` in a `StatRow` stay as they are; an `EmptyState` belongs where rows, cards or a figure would have been.
  - `size="sm"` is the density inside a table body or a panel. A screen never builds the failure branch itself: `TableState` checks it before the emptiness, and only when there are no rows, so a refetch that failed keeps the good ones.
- **"None match" and "there are none yet" are different facts.** `ListView` takes `empty` and `emptyFiltered`; the wrong one sends an admin looking in the wrong place. Never rebuild that ternary at a call site. Both take `string | EmptyCopy` — a bare string is the TITLE, so it is a plain noun with no full stop, and a second sentence is a `hint`. `React.ReactNode` was removed deliberately: passing an `Alert` is now a type error.
...
- **The nav never takes space from the page.** The icon rail is the resting state and the only nav in the layout flow, so the content region is a constant. **Pointing at the rail widens it**, and leaving collapses it: the widening layer is absolutely positioned inside the rail's own width, so it floats over the page with no scrim, no header of its own and nothing to close. Never widen a flex sibling of `<main>`. It does NOT open on focus — swapping the rows out from under the row that has focus loses it, so a keyboard reads the collapsed rail with its tooltips and popovers. Below the rail breakpoint there is no rail, so the header's button opens a `Sheet`, which is modal and owns the focus trap. In the panel one section opens at a time; the rail, and a section past `NAV_INLINE_MAX_ITEMS`, open a popover beside the row. **On touch the panel drills down** — a popover over a sheet is a modal over a modal.
...
- **The first column is the record's OWN name; what it hangs off comes after it.** A test list leads with the test, a configuration list with the configuration — never with the stage, however useful the stage is for scanning. The reader came for the rows, and a row that opens with its parent reads as a list of parents. The parent is a column like any other, and `StageCell` is the one way it is drawn.
