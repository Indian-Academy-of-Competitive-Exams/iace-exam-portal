...
- **A region with nothing to show is an `EmptyState`; an `Alert` is for data that IS there and needs attention.** A blue box saying "no events yet" is the wrong instrument — it reads as something to act on, and the reader hunts for the action. There are exactly **three** absences and the glyph names which one before a word is read: `EMPTY` (Inbox) nothing yet, `FILTERED` (SearchX) nothing matches, `FAILURE` (CircleX, crimson) it did not load. `kind` picks the icon — **a per-screen glyph is decoration**, restating a heading that already said it, and `icon` is overridden only where the absence means something else ("No mistakes recorded" is a good outcome and takes a check).
  ...
  - **A failure names an EVENT, so its heading is a sentence** — "Could not load this test". That is the one exemption in `no-narration`; `EMPTY` and `FILTERED` name a region and stay plain nouns.
