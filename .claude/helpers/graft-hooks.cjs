...
const BAKED =
  '/Users/harshithdiyyala/.nvm/versions/node/v22.23.2/lib/node_modules/@nanonets/graft/dist/claude';
...
function versionOf(distClaude) {
  try {
    return (
      JSON.parse(fs.readFileSync(path.join(distClaude, '..', '..', 'package.json'), 'utf8'))
        .version || null
    );
  } catch {
    return null;
...
function best(dirs, name) {
  ...
  for (const d of dirs) {
    ...
    const v = versionOf(d);
    if (bestDir === null || newer(v, bestVer)) {
      bestDir = d;
      bestVer = v;
    }
  }
...
import(pathToFileURL(entry('hooks.js')).href)
