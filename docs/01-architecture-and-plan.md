...
- **Subject → Topic → SubTopic** — the taxonomy. A topic belongs to one subject; a **sub-topic is shared many-to-many across topics** (and so across subjects, only ever *through* a topic — it has no Subject relation). Names are unique table-wide so the same sub-topic is one row everywhere it is taught.
