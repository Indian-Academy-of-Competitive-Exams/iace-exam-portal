...
- [ ] `finalize` increments `Question.fixedUseCount` once per question on the paper. Nothing
...
- [ ] One `thaw(tx, test)` that applies `unfreezing()` AND decrements the counts for the paper the
      test currently holds. Both callers use it; neither writes the patch by hand.
- [ ] Never below zero: a draft that was never finalized has nothing to give back.
...
- [ ] `PATCH /admin/tests/:id/paper/:rowId` takes `{ questionId }` and swaps the question on one
      row, keeping its `order` and pinning the new question's `currentVersionId`.
- [ ] `DELETE /admin/tests/:id/paper/:rowId` drops one row. Its `order` leaves a gap, which is
      what `@@unique([testId, order])` allows and what leaves the section reading Short.
- [ ] Both refuse once the test has been sat, and both thaw a frozen one through Task 1's `thaw`.
- [ ] The replacement must belong to the section's subject and must not already be on the paper —
...
- [ ] `useListScreen` takes an optional filter store, defaulting to `useFilters()`. A dialog's
      filters are not a link anyone shares, and one section's topic filter must not survive into
      the next section's dialog.
- [ ] `useLocalFilters` implements the same four methods in component state.
...
- [ ] A dialog per section: search, topic, difficulty and tag filters, a checkbox per row, and the
...
- [ ] The topic filter is narrowed to the section's subject, since that is what the draw will use.
- [ ] The footer says how many are chosen against how many the section needs.
...
- [ ] Each section expands to the questions actually on the paper, in order, with code, stem,
      topic and difficulty. `DataTable`'s `expand` gives the panel its own cap and scroll.
- [ ] One `RowActions` menu per question: Replace (opens the picker for one) and Remove
      (destructive, confirmed).
- [ ] Draw again moves into a `ConfirmDialog` naming what it replaces and, on a frozen test, that
...
- [ ] The accordion strip goes. The configuration's name becomes a button in the header opening a
      `Dialog` holding the configuration AND a per-section table: questions, marks each, negative,
      duration, merit or qualifying.
- [ ] `text-sm` throughout, as everywhere else. The `text-xs` was a strip's compromise and a
      dialog has room.
- [ ] Twelve alerts become six. Going: "Drawing again replaces every question below", "drawing
...
- [ ] Correct the two strings that say finalizing locks the base configuration. It does not, and
