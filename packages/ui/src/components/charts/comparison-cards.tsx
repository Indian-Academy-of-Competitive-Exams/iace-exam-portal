    ...
    <ul className={cn('grid gap-4 sm:grid-cols-2 lg:grid-cols-3', className)}>
      {items.map((item) => (
        <li key={item.key}>
          <Card
            className={cn(
              'flex h-full flex-col gap-3 bg-chart-surface p-5',
              EDGE[item.tone ?? 'neutral'],
            )}
          >
            ...
            <CompositionBar size="sm" segments={item.segments} aria-label={item.label} />

            <p className="text-xs text-foreground-secondary">{item.caption}</p>
          </Card>
        </li>
      ))}
    </ul>
