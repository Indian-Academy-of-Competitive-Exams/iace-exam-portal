...
describe('tables', () => {
  it('offers a table on a field that holds blocks', () => {
    ...
    assert.ok(screen.getByLabelText('Insert a table'));
  });
  ...
  it('offers none on a single-line field', () => {
    ...
    assert.equal(screen.queryByLabelText('Insert a table'), null);
  });

  it('inserts a real table, with a header row', () => {
    let html = '';
    show(<RichText value="" onChange={(next) => (html = next)} />);
  ...
  /** The failure this prevents: one menu for whichever table the caret was in, and no second table. */
  it('leaves rows, columns and delete to the table itself', () => {
    show(<RichText value="" onChange={noop} />);

    fireEvent.click(screen.getByLabelText('Insert a table'));

    assert.ok(screen.getByRole('button', { name: 'Add a row below' }));
