...
//
// A sibling module is reachable only through its PUBLIC ENTRY — its
// `<module>.module.ts` or an `index.ts` barrel — because a deep import is a
// silent coupling that blocks lifting that module into its own service later.
// Infra (`prisma`, `redis`, `queue`, `storage`, `config`, `common`) is exempt — shared libraries every service links, not bounded contexts (§4.4).
import path from 'node:path';

/** Shared libraries, not bounded contexts — these never become services, so a deep import into one costs nothing at extraction time. */
...
/** Everything after `<root>/src/`, split into segments — or null for a file not under a `src` directory at all (a config file, a test). */
...
/** `apps/api/src` module boundaries: no deep sibling imports. @type {import('eslint').Rule.RuleModule} */
export const apiModuleBoundaries = {
  ...
  create(context) {
    ...
    // Files directly in `src/` are the composition root — `app.module` wires everything and `main` boots it, so they may see everything.
    ...
    function check(node) {
      ...
      // The public entry, in the two shapes a Nest module has: the barrel (index.ts) or the module file (auth.module).
