  ...
  for (const key of DATE_COLUMN_KEYS) {
    ...
    for (let row = 2; row <= IMPORT_MAX_ROWS + 1; row += 1) {
      sheet.getCell(row, index + 1).dataValidation = {
        ...
        prompt: 'Type it however you normally do. Any common format is read.',
...
const STUDENT_IMPORT_NOTES = [
  ...
  ['mobile number. Tell them to change it when they first sign in, because anyone holding'],
  ...
  [''],
  ['Enrolled Courses: a whole course (SSC, RRB, BANKING, AP/TS POLICE), for a student'],
  ...
  [''],
  ['Date of Birth: any common format. 11/04/2003, 2003-04-11, 11-Apr-2003 and whatever your'],
  ['Excel reformats it to are all read the same. Day comes first when it could go either way,'],
