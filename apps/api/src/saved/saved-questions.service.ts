...
import { gateFacts, solutionsAreOpen, solutionsClosedReason } from '../attempts';
...
export class SavedQuestionsService {
  ...
  /** The star rides the solution gate: it is offered on the review surface, which opens with it. */
  private async servedPastTheGate(
    ...
    const schedule = await this.access.testSchedule(served.attempt.testId);
    const facts = gateFacts(served.attempt, schedule);
