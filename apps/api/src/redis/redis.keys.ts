...
/** Every Redis key the platform uses is minted here, so the keyspace stays greppable and no two features collide on a prefix. */
export const redisKeys = {
  ...
  /** The platform-wide catalog bust counter. A series-wide change is one INCR here, and every student's key at the old epoch falls out by its TTL — a SCAN would not survive 100k students. */
  catalogEpoch: 'access:catalog:epoch',

  /** One student's own bust counter. INCR, never DEL: a delete landing between a cache miss and the write behind it re-pins the pre-change answer for the whole TTL. */
