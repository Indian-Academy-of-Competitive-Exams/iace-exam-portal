...
describe('defaultPinFor', () => {
  ...
  /**
   * The failure this prevents: a PIN of the wrong length is one the login schema refuses, so an
   * imported student could not sign in at all — and the import would report success for every one of
   * them.
   */
  ...
  /**
   * Every mobile the importer can accept must yield a usable PIN, so the two schemas are checked
   * against each other rather than against a fixed list.
   */
