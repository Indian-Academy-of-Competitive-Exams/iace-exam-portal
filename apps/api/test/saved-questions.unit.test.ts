...
function build(
  rows: FakeSavedRow[] = [],
  attempts: FakeSavedAttempt[] = [makeSavedAttempt()],
  closesAt: string | null = CLOSED,
) {
  const prisma = new FakeSavedPrisma(rows, attempts);
  return {
    prisma,
    saved: new SavedQuestionsService(prisma.asService(), fakeTestSchedule(closesAt)),
  };
}
...
describe('SavedQuestionsService — starring a question', () => {
  ...
  /** The star is offered on the review surface, so it rides the same gate the answer key does. */
  it('refuses while the test is still open for somebody to sit', async () => {
    const { saved } = build(
      [],
      [makeSavedAttempt({ evaluationMode: EVALUATION_MODE.RANKED })],
      null,
    );
    ...
    assert.equal(codeOf(error), ErrorCodes.FORBIDDEN);
  });
