...
export default [
  ...
  },
  {
    files: ['src/**/*.ts'],
    rules: {
      // MUST stay off for NestJS. With `emitDecoratorMetadata`, TypeScript emits constructor parameter types as design:paramtypes VALUES — that metadata is how the DI container resolves a provider, so rewriting an injected class to `import type` erases it and breaks injection at runtime with no compile-time warning.
